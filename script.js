...
entryForm.addEventListener("submit", (e) => {
  ...
  entryForm.style.display = "none"; //hides the entry form box
  categoryBox.style.display = "block"; //displays the categories section box
});

//event listener for handling the changes in the category choice
selectedCategory.addEventListener("change", async () => {
  ...
  // if it is a new category, do the following
  try {
    const response = await fetch(
      `https://the-trivia-api.com/v2/questions?limit=30&categories=${category}`
    ); //here large amount questions are fetched
    ...
    ]; //questions are filtered based on their difficulty and spreaded into a new array called selectedQuestions
    ...
    categoryBox.style.display = "none"; //hides the category box
    questionContainer.style.display = "block"; //displays the question box
...
//this function updates the score for each player and triggers the display of next question
function handleAnswer(selectedAnswer, question) {
  ...
  currentPlayerIndex = (currentPlayerIndex + 1) % 2; // Switch players --> if player is 0 --> 0+1 = 1%2 = 1 --> if player is 1 -->1+1%2 = 0
...
function endGame() {
  ...
  // Show the result section with scores
  resultText.innerHTML = `${winner} <br>Scores:<br> ${players[0].name}: ${players[0].score}<br> ${players[1].name}: ${players[1].score}`;
